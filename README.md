...
- Tooling config is overlaid after the managed block. The launcher first loads the **OmniRoute MCP/skills catalog** via `tools/omniroute-catalog.mjs` (remote `OMNIROUTE_CATALOG_URL` with Bearer auth → last-known-good `.codex-omniroute-catalog-cache.json` → bundled `default-mcp-catalog.json` in the repo), then imports `[marketplaces.*]`, `[plugins.*]`, `[mcp_servers.*]`, `[projects.*]`, `[windows]`, and `[profiles.omniroute_managed.windows]` from the previous isolated config, then from your real `~/.codex/config.toml`, then from optional `codex-omniroute-config-overlay.toml` next to the launcher. Later sources win on name collision — so your local config always overrides catalog defaults, and the overlay always overrides everything. The managed `omniroute_bridge` provider/profile cannot be overridden.
- The catalog also writes skills into `<isolated-home>/skills/<name>/SKILL.md` so every install gets a working MCP/skills baseline on first launch with no per-user setup. Set `OMNIROUTE_CATALOG_DISABLED=1` in `.env` to skip the helper entirely.
