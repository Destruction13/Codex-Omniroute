...
.DESCRIPTION
    Walks a non-technical user through the entire install in one go:
      ...
      2. Asks for the OmniRoute base_url and API key (interactive prompt).
         The API key is read with -AsSecureString so it does not echo to
         the terminal. Both values land in `omniroute-provider.json`,
         which is gitignored.

      3. Optionally asks for the OmniRoute model_prefix (default "cx/")
         and whether to enable GPT-5.5 connection-ID pinning.

      4. Runs `verify-codex-omniroute.ps1 -NoCodex` so the user gets a
         real PASS/FAIL summary before they ever launch Codex.

      5. Tells the user exactly which .bat to double-click next.

    All choices are persisted in `omniroute-provider.json` (NEVER committed)
    so re-running Setup.bat overwrites previous values cleanly.
...
function Save-ProviderJson {
    param(
        ...
        [string]$ModelPrefix,
        [bool]$Pin55Enabled,
        [string]$Pin55ConnectionId
    )

    $obj = [ordered]@{
        '_comment'      = 'Generated by Setup.ps1. Edit by hand or rerun Setup.bat. NEVER commit this file.'
        'base_url'      = $BaseUrl
        'api_key'       = $ApiKey
        ...
        'gpt55_pin'     = [ordered]@{
            '_comment'      = 'Used only when OMNIROUTE_PIN_55=1 in env, OR enabled=true here.'
            'enabled'       = $Pin55Enabled
            'connection_id' = $Pin55ConnectionId
...
if ($NonInteractive) {
    ...
    if (Test-Path -LiteralPath $providerPath) {
        ...
        $overwrite = Read-YesNo -Prompt 'Overwrite it?' -DefaultYes $false
        if (-not $overwrite) {
            Write-Hint 'Keeping existing omniroute-provider.json untouched.'
        }
    }
    $shouldWrite = (-not (Test-Path -LiteralPath $providerPath)) -or $overwrite
    ...
    if ($shouldWrite) {
        ...
        $prefix  = Read-Required -Prompt 'Model prefix' -Default 'cx/'

        $pinEnabled = Read-YesNo -Prompt 'Enable GPT-5.5 connection-ID pin?' -DefaultYes $false
        $pinId = ''
        if ($pinEnabled) {
            $pinId = Read-Required -Prompt 'GPT-5.5 connection_id'
        }
        ...
        Save-ProviderJson `
            ...
            -ApiKey $apiKey `
            -ModelPrefix $prefix `
            -Pin55Enabled $pinEnabled `
            -Pin55ConnectionId $pinId
...
if ($SkipVerify) {
    ...
    if (-not $psExe) {
        Write-Bad 'No PowerShell host found.'
        Pause-IfInteractive
        exit 2
    }

    & $psExe -NoProfile -File $verifier
