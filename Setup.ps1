...
.DESCRIPTION
    Walks a non-technical user through the entire install in one go:
      ...
      4. Runs `verify-codex-omniroute.ps1 -NoCodex` so the user gets a
         real PASS/FAIL summary before they ever launch Codex. A
         verifier crash never breaks Setup -- the config has already
         been written, and the user can always proceed to
...
function Save-ProviderJson {
    ...
    # default_model, or gpt55_pin can edit omniroute-provider.json by
    # hand -- the wizard never asks about them.
    $obj = [ordered]@{
        '_comment'      = 'Generated by Setup.ps1. Edit by hand or rerun Setup.bat. NEVER commit this file. Advanced fields (model_prefix, default_model, gpt55_pin) use safe defaults -- change them only if you know you need to.'
        'base_url'      = $BaseUrl
        'api_key'       = $ApiKey
        'model_prefix'  = 'cx/'
